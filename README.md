...
所谓 `数据塑形` 就是不同请求，根据请求所需要的字段来返回 JSON 数据。减少不必要的网络支出和数据冗余。通过 param 传入所需要的字段 fields=orderID,productID 返回 JSON 数据就只包含 fields 中所传入的字段。当然，使用 HATEOS 还会含有 links。在 Java 中通过反射，塑形后放入 Map，返回 JSON 给前端。
